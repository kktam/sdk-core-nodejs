...
function OAuth(consumerKey, p12PathOrFunc, alias, password) {
    ...
    var callback = function(p12FileData) {
        //console.log("callback run with p12FileData length = " + p12FileData.length);

        // record private key in local space
        privateKey = _getPrivateKey(p12FileData, alias, password);  
    }

 ...
 *
 * @param consumerKey - consumer key from MasterCard Developer Zone
 * @param p12File - string content of p12 key store
 * @param alias - key alias for p12 key
 * @param password - password for p12 key
 * @constructor
 */
function OAuthWithKeyStore(consumerKey, p12File, alias, password) {
    if(!utils.isSet(consumerKey)) {
        throw new error.SDKError('Consumer key is not set');
    }

    if(!p12File || p12File.length <= 1) {
        throw new error.SDKError('p12File key store content is empty');
    }
 ...
 *
 * @param p12File - p12 key store content
 * @param alias
 * @param password
 * @returns {*} PEM
 * @private
...
module.exports = {
    OAuth: OAuth,
    OAuthWithKeyStore: OAuthWithKeyStore
};
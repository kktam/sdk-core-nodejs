...
function OAuth(consumerKey, p12PathOrFunc, alias, password) {
    ...
    this.callback = function(p12FileData) {
        console.log("callback run with p12FileData length = " + p12FileData.length);
        this.privateKey = _getPrivateKey(p12FileData, alias, password);  
    }
    ...
    if (typeof p12PathOrFunc === 'string') {
        ...
        p12Promise = p12PathOrFunc(this.callback);
    }
    ...
    this.sign = function (uri, httpMethod, body) {
        if (p12Promise == null) {
            // do not need to wait
            console.log("regular");
            return _sign(uri, httpMethod, body);
        } else {
            // wait for the Promise provided
            console.log("promise");        
            p12Promise.then(function (data) {
                return _sign(uri, httpMethod, body);
            }, function (data) {
                // TBD - log error 
            });
...
function check(uri, httpMethod, body) {
}

 ...
 */
var _sign = function (uri, httpMethod, body) {
 ...
 * @param p12PathOrFunc - path to .p12 file, or function to provide the method to obtain the p12 file contents
 *                        the function prototype is p12PathOrFunc(callback) where the function takes no parameters.
 *                        when the function succeed, it must call the callback and pass in UInt8Array representation of p12 file contents.
 * @param alias - key alias for p12 key
 * @param password - password for p12 key
 * @constructor
 */
function OAuth(consumerKey, p12PathOrFunc, alias, password) {
    ...
    if (typeof p12PathOrFunc === 'string') {
        if(!utils.isSet(p12PathOrFunc)) {
            throw new error.SDKError('Path to .p12 file is not set');
        }

        // Read p12 file
        p12File = fs.readFileSync(p12PathOrFunc, 'binary');      
        this.privateKey = _getPrivateKey(p12File, alias, password);

        privateKey = this.privateKey;
        p12Promise = null;       
    } else if (typeof p12PathOrFunc === 'function') {
        p12Promise = p12PathOrFunc(consumerKey, null);
        this.promise = p12Promise;
        ...
        p12Promise.then(function(result) {
            ...
            // record private key
            var privateKey = _getPrivateKey(result.p12, alias, password); 
        
            // store all result in cache            
            cache.set(result.consumerKey, {
...
/**
 * Return the private key from the most recent oauth configuration
 * @param consumerKey - the consumer key provided from OAuth()
 * @returns Authentication object
 */
function getAuthentication(consumerKey) {   
    var authentication = cache.get(consumerKey);
    if (authentication != null) {
        authentication['sign'] = _sign;
    }
 ...
 * @private
 */
var _getPrivateKey = function (p12File, alias, password) {
    // Get asn1 from DER
...
/**
 * Internally perform the signing of the HTTP Request and return OAuth header value
 * (currently used)
 * @param uri
 * @param httpMethod
 * @param body
...
var _sign = function (uri, httpMethod, body) {
    ...
    // wait for private key to arrive
    //console.log('private key length = ' + privateKey.length);

    return _signRequest(oauthParams, privateKey, signatureBaseString);
};
